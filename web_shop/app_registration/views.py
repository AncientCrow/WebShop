...
from app_registration import forms, models, serializers, filters
...
class ProfileDetail(View):
    ...
    def get_random_product():
        ...
        while True:
            ...
            goods = Product.objects.filter(pk=pk).first()
            if goods:
                return goods
            else:
                return None
    ...
    def get(self, request, pk):
        try:
            ...
            product_1 = cache.get_or_set("product1", self.get_random_product(), 3 * 60)
            product_2 = cache.get_or_set("product2", self.get_random_product(), 3 * 60)
            product_3 = cache.get_or_set("product3", self.get_random_product(), 3 * 60)
            product_4 = cache.get_or_set("product4", self.get_random_product(), 3 * 60)
            author_products = models.Profile.objects.annotate(
